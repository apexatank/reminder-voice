...
import { Mic, Play, CalendarCheck, BrainCircuit, CheckCircle2, Menu, ChevronDown, Check, Zap, Smartphone, Globe, Shield, Clock } from 'lucide-react';
...
export default function Home() {
  ...
  React.useEffect(() => {
    ...
    const testimonialsLength = 6;
    const interval = setInterval(() => {
      setActiveIndex((prev) => (prev + 1) % (window.innerWidth < 1024 ? testimonialsLength : testimonialsLength - 2));
    }, 5000);
  ...
  return (
    <div className="min-h-screen bg-[#0E061B] text-slate-50 relative font-sans selection:bg-purple-500/30">
      ...
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isMenuOpen ? 'bg-[#0E061B]' : 'bg-[#0E061B]/60 backdrop-blur-xl'} border-b border-white/5`}>
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-3 group cursor-pointer" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
            ...
            <span className="font-bold text-2xl tracking-tight bg-clip-text text-transparent bg-linear-to-r from-white to-slate-400">Remind Voice</span>
          </div>
 ...
 <div className="absolute top-150 right-80 animate-bounce-slow delay-700 opacity-20 hidden lg:block">
        ...
        >
          <div className="flex-1 text-center md:text-left z-10">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center md:justify-start gap-5">
              <button className="w-full sm:w-auto flex items-center justify-center gap-3 bg-linear-to-r from-[#7042f8] to-[#925ffc] hover:opacity-90 transition-all text-white px-10 py-5 rounded-full font-bold text-xl shadow-[0_0_40px_-10px_#7042f8] hover:scale-105 active:scale-95">
                <Mic className="w-6 h-6" />
                Start Speaking
              </button>
              <button className="w-full sm:w-auto flex items-center justify-center gap-3 bg-white/5 hover:bg-white/10 border border-white/10 transition-all text-white px-10 py-5 rounded-full font-bold text-xl backdrop-blur-md hover:scale-105 active:scale-95">
        ...
        {/* Brand/Trust Bar */}
        <div className="max-w-7xl mx-auto px-6 py-20 text-center opacity-40">
           <p className="text-xs font-bold tracking-widest uppercase mb-10 text-slate-500">TRUSTED BY TEAMS AT</p>
           <div className="flex flex-wrap justify-center items-center gap-12 md:gap-24">
              <span className="text-2xl font-bold italic tracking-tighter">VOLT</span>
              <span className="text-2xl font-black tracking-tight">ECHO</span>
              <span className="text-2xl font-medium uppercase tracking-[0.2em]">Flux</span>
              <span className="text-2xl font-black">NEXA</span>
              <span className="text-2xl font-bold tracking-widest">ORBIT</span>
           </div>
        </div>
             ...
             <div className="flex-1 relative w-full h-[500px] flex items-center justify-center">
                ...
                <div className="relative w-72 h-72 md:w-96 md:h-96">
                   ...
                   <div className="absolute inset-8 bg-linear-to-tr from-purple-500/40 to-blue-500/10 rounded-[2.5rem] -rotate-12 border border-white/20 animate-pulse" />
                    ...
                    {[
                       { icon: <Smartphone className="w-6 h-6 text-blue-400" />, pos: "top-[-30%] left-[-15%]" },
                       { icon: <Shield className="w-6 h-6 text-purple-400" />, pos: "top-[-20%] right-[-25%]" },
                       { icon: <Clock className="w-6 h-6 text-orange-400" />, pos: "bottom-[-20%] right-[10%]" },
                       { icon: <CalendarCheck className="w-6 h-6 text-green-400" />, pos: "bottom-[10%] left-[-30%]" },
                       { icon: <Mic className="w-6 h-6 text-red-400" />, pos: "bottom-[40%] right-[-35%]" },
                    ].map((item, i) => (
                       <motion.div 
                         key={i}
                         animate={{ 
                         ...
                         className={`absolute ${item.pos} w-20 h-20 bg-white/10 backdrop-blur-2xl border border-white/20 rounded-3xl flex items-center justify-center shadow-[0_0_30px_rgba(138,92,245,0.2)] z-20 hover:scale-110 transition-transform cursor-default group`}
                       >
        ...
        <motion.section 
          ...
          id="how-it-works" className="max-w-7xl mx-auto px-6 py-32 border-t border-white/5 relative"
        >
        ...
        <motion.section 
          ...
          id="features" className="max-w-7xl mx-auto px-6 py-32 flex flex-col xl:flex-row items-center gap-24"
        >
          ...
          <div className="flex-1 w-full relative h-[500px] md:h-[600px] flex items-center justify-center">
            <div className="flex justify-center items-end gap-2 sm:gap-6 relative z-20 scale-75 sm:scale-90 md:scale-100">
              ...
              {/* Left Phone */}
              <div className="hidden sm:block w-48 md:w-56 h-[400px] md:h-[440px] bg-[#0A0512] rounded-[2.5rem] border-[6px] border-[#2A2045] shadow-2xl relative overflow-hidden translate-y-12 opacity-80 z-10 transition-all duration-500 hover:-translate-y-4 hover:rotate-0 transform -rotate-12 group">
                <div className="absolute top-0 w-full h-5 bg-black flex justify-center items-end pb-1">
                  <div className="w-20 h-3 bg-[#0A0512] rounded-b-lg" />
                </div>
                <div className="p-4 pt-8 h-full flex flex-col">
                   <div className="h-full bg-linear-to-b from-white/5 to-transparent rounded-2xl p-4 overflow-hidden">
                      <div className="w-full h-4 bg-white/10 rounded-full mb-6" />
                      <div className="space-y-3">
                         {[
                            { title: "Team Sync", time: "10:00 AM", color: "text-blue-400" },
                            { title: "Grocery Shop", time: "06:30 PM", color: "text-purple-400" },
                            { title: "Call Dad", time: "08:00 PM", color: "text-green-400" }
                         ].map((item, i) => (
                           <div key={i} className="p-3 bg-white/5 rounded-xl border border-white/5 space-y-1 group-hover:bg-white/10 transition-colors">
                              <p className={`text-[8px] font-bold uppercase tracking-widest ${item.color}`}>{item.time}</p>
                              <p className="text-[10px] font-bold text-white/80">{item.title}</p>
                           </div>
                         ))}
                      </div>
                   </div>
              ...
              <div className="hidden sm:block w-48 md:w-56 h-[400px] md:h-[440px] bg-[#0A0512] rounded-[2.5rem] border-[6px] border-[#2A2045] shadow-2xl relative overflow-hidden translate-y-12 opacity-80 z-10 transition-all duration-500 hover:-translate-y-4 hover:rotate-0 transform rotate-12 group">
                ...
                <div className="p-4 pt-8 h-full flex flex-col">
                   <div className="h-full bg-linear-to-b from-white/5 to-transparent rounded-2xl p-4 overflow-hidden">
                      ...
                      <div className="space-y-3">
                         {[
                            { title: "Dentist", date: "TUE, 14 MAR", color: "text-orange-400" },
                            { title: "Flight NYC", date: "WED, 15 MAR", color: "text-blue-400" },
                            { title: "Deadline", date: "FRI, 17 MAR", color: "text-red-400" }
                         ].map((item, i) => (
                           <div key={i} className="p-3 bg-white/5 rounded-xl border border-white/5 space-y-1 group-hover:bg-white/10 transition-colors">
                              <p className={`text-[8px] font-bold uppercase tracking-widest ${item.color}`}>{item.date}</p>
                              <p className="text-[10px] font-bold text-white/80">{item.title}</p>
                           </div>
                         ))}
                      </div>
          ...
          <div className="max-w-7xl mx-auto px-6">
            ...
            <div className="relative group overflow-hidden">
              <motion.div 
                ...
                animate={{ x: isMobile ? `calc(-${activeIndex * 100}% - ${activeIndex * 40}px)` : `calc(-${activeIndex * 33.333}% - ${activeIndex * 27}px)` }}
              ...
              >
                {[
                  ...
                  <div 
                    key={i} 
                    className="w-full md:w-[calc(33.333%-27px)] shrink-0 bg-white/2 bg-linear-to-tr from-white/10 to-transparent border border-white/10 p-10 rounded-[2.5rem] backdrop-blur-xl relative overflow-hidden"
                  >
                    ...
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-2xl shrink-0 shadow-lg relative overflow-hidden bg-linear-to-br from-purple-500/20 to-blue-500/20">
                        <img 
        ...
        >
          <div className="max-w-7xl mx-auto px-6">
             ...
             <div className="relative max-w-5xl mx-auto aspect-video md:aspect-21/9 bg-white/2 rounded-[3.5rem] border border-white/10 overflow-hidden flex items-center justify-center group shadow-2xl">
                ...
                <div className="relative z-10 flex flex-col md:flex-row items-center gap-12 md:gap-24 p-8">
                   ...
                   <div className="space-y-6">
                      <div className="p-6 bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl hover:bg-white/10 transition-all cursor-default">
                         <p className="text-slate-500 text-[10px] font-bold uppercase tracking-widest mb-1">Latency</p>
                         <p className="text-3xl font-black text-white">42<span className="text-xs text-blue-400 ml-1">ms</span></p>
                      </div>
                      <div className="p-6 bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl hover:bg-white/10 transition-all cursor-default translate-x-4">
                         <p className="text-slate-500 text-[10px] font-bold uppercase tracking-widest mb-1">Accuracy</p>
                         <p className="text-3xl font-black text-white">99.9<span className="text-xs text-purple-400 ml-1">%</span></p>
                      </div>
                   </div>
                   ...
                   <div className="space-y-6">
                      <div className="p-6 bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl hover:bg-white/10 transition-all cursor-default -translate-x-4">
                         ...
                         <p className="text-sm font-bold text-white/80">Cross-Platform Ready</p>
                      </div>
                      <div className="p-6 bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl hover:bg-white/10 transition-all cursor-default">
                         <p className="text-slate-500 text-[10px] font-bold uppercase tracking-widest mb-1">Languages</p>
                         <p className="text-xl font-black text-white">40+ SUPPORTED</p>
                      </div>
                   </div>
                </div>
      ...
      >
        ...
        <div className="max-w-6xl mx-auto px-6 text-center z-10 relative">
          ...
          <div className="mt-40 pt-16 border-t border-white/5 flex flex-col md:flex-row items-center justify-between text-xs font-bold uppercase tracking-widest text-slate-600 gap-8">
            <p>© 2024 Remind Voice. Future of Productivity.</p>
            <div className="flex flex-wrap justify-center gap-10">
              {['Privacy', 'Terms', 'Contact', 'Ecosystem'].map((item) => (
                item === 'Ecosystem' ? (
                  <a 
                    key={item} 
                    href="#features" 
